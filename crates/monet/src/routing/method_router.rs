...
impl<S, E> fmt::Debug for MethodRouter<'_, S, E> {
    ...
    S: Clone + 'a,
{
    ...
    pub fn with_state<S2>(self, state: &'a S) -> MethodRouter<'a, S2, E> {
        MethodRouter {
    ...
    pub(crate) fn merge_for_path(
        ...
        fn merge_inner<'b, S, E>(
            path: Option<&str>,
            name: &str,
            first: MethodEndpoint<'b, S, E>,
            second: MethodEndpoint<'b, S, E>,
        ) -> Result<MethodEndpoint<'b, S, E>, String> {
            match (first, second) {
...
impl<S, E> fmt::Debug for MethodEndpoint<'_, S, E> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl<S, E> Clone for MethodEndpoint<'_, S, E> {
    fn clone(&self) -> Self {
        match self {
            Self::None => Self::None,
