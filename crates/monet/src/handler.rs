    ...
    Res: IntoResponse,
{
    ...
    fn call(self, _req: HttpRequest<'a>, _state: S) -> Self::Future {
...
macro_rules! impl_handler {
    (
        ...
        impl<'a, F, Fut, S, Res, M, $($ty,)* $last> Handler<'a, (M, $($ty,)* $last,), S> for F
        where
            F: FnOnce($($ty,)* $last,) -> Fut + Clone + 'a,
            Fut: Future<Output = Res>,
            S: 'a,
            Res: IntoResponse,
...
#[allow(non_snake_case, unused_mut)]
impl<'a, F, Fut, S, Res, M, T1> Handler<'a, (M, T1), S> for F
where
    F: FnOnce(T1) -> Fut + Clone + 'a,
    Fut: Future<Output = Res>,
    S: 'a,
    Res: IntoResponse,
    T1: FromRequest<S, M>,
...
#[allow(non_snake_case, unused_mut)]
impl<'a, F, Fut, S, Res, M, T1, T2> Handler<'a, (M, T1, T2), S> for F
where
    F: FnOnce(T1, T2) -> Fut + Clone + 'a,
    Fut: Future<Output = Res>,
    S: 'a,
    Res: IntoResponse,
...
pub struct HandlerService<'a, H, X, S> {
    ...
    pub(crate) _lt: PhantomData<&'a ()>,
}

impl<'a, H, X, S> Clone for HandlerService<'a, H, X, S>
where
    H: Clone,
    S: Clone,
{
...
impl<'a, H, X, S> HandlerService<'a, H, X, S> {
...
impl<'a, H, T, S> fmt::Debug for HandlerService<'a, H, T, S> {
